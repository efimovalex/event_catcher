...
func (e *GroupResubscribeEvent) MapEvent(eventMap map[string]interface{}) {
	...
	if val, ok := eventMap["user_agent"]; ok {
...
func (e *GroupResubscribeEvent) Save(adaptor *Adaptor) error {
	groupResubscribeTable := adaptor.Session.Table("group_resubscribe_events", &GroupUnsubscribeEvent{}, gocassa.Keys{
		PartitionKeys: []string{"sg_event_id"},
	})

	if err := groupResubscribeTable.Set(e).Run(); err != nil {
