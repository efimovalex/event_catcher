...
func (e *DeliveredEvent) MapEvent(eventMap map[string]interface{}) {
	...
	delete(eventMap, "tls")
	e.CertificateError = eventMap["cert_err"].(string)
...
func (e *DeliveredEvent) Save(adaptor *Adaptor) error {
	deliveredTable := adaptor.Session.Table("delivered_events", &DeliveredEvent{}, gocassa.Keys{
		PartitionKeys: []string{"sg_event_id"},
	})

	if err := deliveredTable.Set(e).Run(); err != nil {
